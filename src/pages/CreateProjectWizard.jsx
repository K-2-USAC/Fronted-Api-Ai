...
import { ArrowRight, ArrowLeft, CheckCircle2, Plus, Trash2, Code2, Paintbrush, Globe, CalendarDays } from 'lucide-react';
...
const CreateProjectWizard = () => {
  ...
  const validateStep = (step) => {
    setError(null);
    if (step === 1) {
      if (!wizardData.name.trim()) { setError('El nombre del proyecto es obligatorio'); return false; }
      if (!wizardData.type.trim()) { setError('El tipo de negocio es obligatorio'); return false; }
    }
    if (step === 2) {
      if (!wizardData.description.trim()) { setError('La descripción es obligatoria'); return false; }
    }
    if (step === 4) {
      if (!wizardData.agentId.trim()) { setError('El Agent ID es obligatorio para la integración'); return false; }
    }
    return true;
  };
            ...
            return (
              <div key={step.id} className="relative z-10 flex flex-col items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 ${
                  ...
                  isCurrent ? 'bg-white text-dark shadow-[0_0_15px_rgba(255,255,255,0.5)]' : 
            ...
            {currentStep === 1 && (
              <div className="space-y-6 flex-1">
                <div>
                  ...
                  <p className="text-white/50 mb-8">What are we building today?</p>
                </div>
            ...
            {currentStep === 2 && (
              <div className="space-y-6 flex-1">
                <div>
                  ...
                  <p className="text-white/50 mb-8">This helps our AI understand your goals.</p>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe what your business does and what you want to achieve..."
                  />
                </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-6 flex-1">
                <div>
                  ...
                  <p className="text-white/50 mb-8">Choose the vibe of your application.</p>
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-white/70 mb-4">Tono de Voz</label>
                  <div className="grid grid-cols-2 gap-4">
                    {['Formal', 'Amigable', 'Profesional', 'Casual', 'Persuasivo'].map(tone => (
                      <div 
                        key={tone}
                ...
                <div>
                  <label className="block text-sm font-medium text-white/70 mb-2 mt-4">Idioma del Agente</label>
                  <select
                    value={wizardData.language}
                    onChange={(e) => setWizardData({ language: e.target.value })}
                    className="input-field appearance-none"
                  >
            ...
            {currentStep === 4 && (
              <div className="space-y-6 flex-1">
                <div>
                  ...
                  <p className="text-white/50 mb-8">Add any extra information.</p>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-2">Agent ID (Twilio/Groq)</label>
                    <input
                      type="text"
                      value={wizardData.agentId}
                      onChange={(e) => setWizardData({ agentId: e.target.value })}
                      className="input-field"
                      placeholder="e.g. agente-123"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-2">Horarios de Atención</label>
                    <input
                      type="text"
                      value={wizardData.businessHours}
                      onChange={(e) => setWizardData({ businessHours: e.target.value })}
                      className="input-field"
                ...
                <div>
                  <label className="block text-sm font-medium text-white/70 mb-4 flex justify-between items-center">
                    ...
                    <button type="button" onClick={addFaq} className="text-accent hover:text-white flex items-center gap-1 text-xs transition-colors">
